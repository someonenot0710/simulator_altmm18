 ...
 */
class DudpServerHelper
{
public:
  /**
   * Create DudpServerHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   * \param port The port the server will wait on for incoming packets
   */
  DudpServerHelper (uint16_t port);
  ...
  /**
   * Create a DudpServerApplication on the specified Node.
   *
  ...
  /**
   * Create a DudpServerApplication on specified node
   *
...
private:
  /**
   * Install an ns3::DudpServer on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an DudpServer will be installed.
 ...
 */
class DudpClientHelper
{
public:
  /**
   * Create DudpClientHelper which will make life easier for people trying
   ...
   */
  DudpClientHelper (Address ip, uint16_t port);
  /**
   * Create DudpClientHelper which will make life easier for people trying
   * to set up simulations with echos. Use this variant with addresses that do
  ...
  /**
   * Given a pointer to a DudpClient application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be DudpClient).
   ...
   *
   * \param node The Ptr<Node> on which to create the DudpClientApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the DudpClientApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
...
private:
  /**
   * Install an ns3::DudpClient on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an DudpClient will be installed.
   * \returns Ptr to the application installed.
   */
  Ptr<Application> InstallPriv (Ptr<Node> node) const;
  ObjectFactory m_factory; //!< Object factory.
