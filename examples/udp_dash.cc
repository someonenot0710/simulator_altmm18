...
#include <fstream>
#include <sstream>
#include "ns3/DudpHelper.h"
#include "ns3/udp-module.h"
...
//     sprintf(tmp_size_file,"%s%s_user%02d_%dx%d_%.1f_gt_%s",size_dir.c_str(),video_names[tmp_user[0]].c_str(),stoi(user_idx),tile_cols,tile_rows,user_bw[cnt],algo.c_str());
     ...
     // udp Jerry
     std::string udp_size_file(tmp2_size_file);
     ifstream udp_size_fp(udp_size_file);

     if(!udp_size_fp)
       cout<<"no size file"<<udp_size_file<<endl;
     vector<double> tmp2_video_size;
     while(getline(udp_size_fp,str)){
         istringstream sss(str);
         string token;
         double tile_size=0;
         while(getline(sss,token,' ')){
         ...
         tmp2_video_size.push_back(tile_size);
     }
     udp_seg_size.push_back(tmp2_video_size);
     udp_size_fp.close();
    // udp Jerry

     // udp Jerry qp2
     std::string udp_size2_file(tmp3_size_file);
     ifstream udp_size2_fp(udp_size2_file);

     if(!udp_size2_fp)
       cout<<"no size file"<<udp_size2_file<<endl;
     vector<double> tmp3_video_size;
     while(getline(udp_size2_fp,str)){
         istringstream sss(str);
         string token;
         double tile_size=0;
         while(getline(sss,token,' ')){
         ...
         tmp3_video_size.push_back(tile_size);
//	cout<<"size: "<< tile_size<<endl; //Jerry
     }
     udp_seg_size2.push_back(tmp3_video_size);

     udp_size2_fp.close();
    // udp Jerry

     std::string udp_size3_file(tmp4_size_file);
     ifstream udp_size3_fp(udp_size3_file);
     if(!udp_size3_fp)
       cout<<"no size file"<<udp_size3_file<<endl;
     vector<double> tmp4_video_size;
     while(getline(udp_size3_fp,str)){
         istringstream sss(str);
         string token;
         double tile_size=0;
         while(getline(sss,token,' ')){
         ...
         tmp4_video_size.push_back(tile_size);
     }
     udp_seg_size3.push_back(tmp4_video_size);
     udp_size3_fp.close();
    // udp Jerry
  ...
  for(uint32_t k=0; k<users; k++){
      ...
      udp_series.push_back(tmp_vseg); //udp Jerry
  }
  for(uint32_t k=0; k<users; k++){
...
*/
  ...
  std::vector<DudpClientHelper> client_udp;
  for (uint32_t user = 0; user < users; user++)
    {
...
//      clientApp.Stop(Seconds(20.0));
      ...
      DudpClientHelper echoClient (user_ipv4[user].GetAddress(1), 9);
      echoClient.SetAttribute ("MaxPackets", UintegerValue (30));
      echoClient.SetAttribute ("Interval", TimeValue (Seconds (1.0)));
      echoClient.SetAttribute ("PacketSize", UintegerValue (100));	
      echoClient.SetAttribute ("VideoId", UintegerValue (user+1));
      ApplicationContainer clientApp_udp = echoClient.Install (nodes.Get (user));
      clientApp_udp.Start (Seconds (0.25));
//      clientApp_udp.Stop (Seconds (20.0));
      client_udp.push_back(echoClient);
      clientApps.push_back(clientApp_udp);
...
//  serverApps.Stop(Seconds(20.0));
  ...
  DudpServerHelper echoServer (9);
  ApplicationContainer serverApps_udp = echoServer.Install (nodes.Get (users));
  serverApps_udp.Start (Seconds (0.0));
//  serverApps_udp.Stop (Seconds (20.0));
